...
void timer_handler(int n) {
    u32 ris, mis, value, load, bload, i, count = 0;
    TIMER* t = &timer[n];
...
int timer_enqueue(PROC* proc, int time)
{
    TQE* newQ, * p;
    int count = 0;
    ...
    if (!tq) {       // when there is nothing in the front of the queue we put it in there first
        tq = newQ;
        kprintf("enqueue %d\n", proc);
        ksleep((int)proc);
        return;
...
int timer_dequeue(int n)
{
    TQE* p;
    int priority;
    p = tq;
    tq = tq->next;      // dequeue it from the list

    kprintf("dequeue %d\n", p->proc);
    priority = p->proc->priority;
    p->proc->priority++; // make it a higher priority;
    kwakeup((int)p->proc);    // wake it up
    p->proc->priority = priority; // set it back to its previous priority

    if (!(p + 1))
        p->next = p + 1;     // free the next node
    else
        p->next = NULL;
...
int timer_printQueue()
{
    TQE* t = tq;
    // print link list as      listname = ...... NULL
    kprintf("TQE = ");
    while (t) {
        kprintf("[%d time:%d]->", t->proc->pid, t->time);
        t = t->next;
    }
    kprintf("NULL\n");
}
