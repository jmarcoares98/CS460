...
int printSleepList(char* listname, PROC* list)
{
    // print link list as      listname = ...... NULL
    kprintf("%s = ", listname);
    while (list) {
        kprintf("[%d%d]->", list->pid, list->exitCode);
        list = list->next;
    }
    kprintf("NULL\n");
}
...
{
  char cmd[64], line[64];
  int i, pid, event, status = 0, waitID, exitValue;
  PROC* p;
  ...
  while(1){
     ...
     kprintf("proc %d running, command = [ps|switch|kfork|ksleep|kwakeup|kwait|time|exit|] : ", running->pid);
     kgets(cmd);
     printf("\n");
     ...
     if (strcmp(cmd, "time") == 0) {
         kprintf("enter seconds: ");
         kgets(line);
         event = atoi(line);
         timer_enqueue(running, event);
         kprintf("\n");
     }
